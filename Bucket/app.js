...
// Erwartung: supabase.js hat window.sb und window.__SB_USER__ gesetzt
if (!window.__SB_READY__) {
  console.warn("Supabase not ready yet. Ensure index.html loads supabase.js + guard before app.js.");
}

// Keys werden nicht mehr genutzt, behalten wir aber zur Kompatibilität in Export-Dateien
...
async function sbGetAccountId() {
  ...
  const { data, error } = await window.sb
    ...
    .eq('user_id', uid)
    .single();
  if (error) { console.warn('sbGetAccountId failed', error); return ""; }
  return data?.account_id || "";
}

async function sbSaveUserProfile({ first, last, avatar }) {
  const uid = window.__SB_USER__?.id;
  if (!uid) return;
  ...
  if (error) {
    console.warn('sbSaveUserProfile failed', error);
  }
}


async function sbUpsert({ state, prefs }) {
  const uid = window.__SB_USER__?.id;
  if (!uid) throw new Error("No user");
...
// ---- Initial Load (statt localStorage chains) ----
...
const storedRemote = await (async () => {
  try { return await sbGetRow(); } catch(e){ console.warn(e); return null; }
  
})();

const prev = storedRemote?.state || null;

const state = normalizeState(prev || { lists: [], selectedListId: null });

// Preferences laden/mergen
const stored = storedRemote?.prefs || {};
const prefs = {
  ...
  profile: {
    ...
    avatar: stored.profile?.avatar || null
  }
};

// **REDEFINE** saveState/savePrefs -> schreiben nach Supabase
async function saveState(){
  try { await sbUpsert({ state }); } catch(e){ console.warn("saveState failed", e); }
}
async function savePrefs(){
  try { await sbUpsert({ prefs }); } catch(e){ console.warn("savePrefs failed", e); }
}
...
function applyPrefs(){
  ...
  setCSS("--bg-url", `url("../assets/background/${prefs.bg}.png")`);
...
const accountFirst         = q("#accountFirst");
const accountLast          = q("#accountLast");
const accountAvatarInput   = q("#accountAvatarInput");
const accountAvatarPreview = q("#accountAvatarPreview");
...
const navVersion       = q("#navVersion");
const settingsCloseX   = q("#settingsCloseX");

let _settingsView = "design"; // default beim Öffnen
...
// -- Konto: Organisationsfeld ausblenden/entfernen (wir zeigen nur E-Mail, Vorname, Nachname, Bild) --
(() => {
  const orgInput = document.getElementById("profileOrg");
  if (!orgInput) return;
  const field = orgInput.closest(".field") || orgInput.parentElement;
  if (field) field.remove();
  // Safety: org im gespeicherten Objekt leeren
  if (prefs?.profile) {
    prefs.profile.org = "";
    savePrefs();
  }
...
function switchSettingsView(view){
  ...
  const isVersion = view === "version";

  const vis = (el, show) => { if (el){ el.hidden = !show; el.style.display = show ? "" : "none"; } };
  vis(settingsDesign,  isDesign);
  vis(settingsMisc,    isMisc);
  vis(settingsAccount, isAccount);
  vis(settingsVersion, isVersion);

  markActiveSettingsNav(view);

  if (isAccount) populateAccountPane(); // ← NEU

  // Version-Titel aktuell halten
  if (isVersion){
...
function populateAccountPane(){
  ...
  // Namen aus prefs
  accountFirst && (accountFirst.value = prefs.profile?.first || "");
  accountLast  && (accountLast.value  = prefs.profile?.last  || "");

  // E-Mail aus Supabase-Session
  const email = (window.__SB_USER__ && window.__SB_USER__.email) ? window.__SB_USER__.email : "";
  accountEmail && (accountEmail.value = email);

  // Konto-ID aus public.users (Cache)
  if (!_accountIdCache) {
    // Failsafe: Falls noch nicht geladen, jetzt nachladen
    // (z.B. wenn Settings sehr früh geöffnet wurden)
    sbGetAccountId().then(id => {
      _accountIdCache = id || "";
      if (accountIdInput) accountIdInput.value = _accountIdCache;
    });
  }
  if (accountIdInput) accountIdInput.value = _accountIdCache || "";
  ...
  if (accountAvatarPreview){
    if (_profileAvatarData) accountAvatarPreview.src = _profileAvatarData;
    else accountAvatarPreview.removeAttribute("src");
  }

  // Copy-Button (falls noch nicht verdrahtet)
  if (accountIdCopyBtn && !accountIdCopyBtn._wired){
    accountIdCopyBtn._wired = true;
    accountIdCopyBtn.addEventListener("click", async ()=>{
      const v = accountIdInput?.value || "";
      if (!v) return;
      try{
        await navigator.clipboard.writeText(v);
        showToast({ title: "Konto-ID kopiert", subtitle: v, duration: 2200 });
      } catch {
...
function buildSubtaskRow(data = { id: uid(), text: "", done: false }) {
  ...
  inputEl.addEventListener("keydown", (ev) => {
    if (ev.key === "Enter" && ev.shiftKey) {
      ev.preventDefault();
      ev.stopPropagation(); // verhindert das Formular-"Speichern" auf Enter

      const container =
        row.parentElement || document.getElementById("subtasksContainer");
      if (!container) return;

      const nextRow = buildSubtaskRow();
      if (row.nextSibling) container.insertBefore(nextRow, row.nextSibling);
      else container.appendChild(nextRow);

      const nextInput = nextRow.querySelector(".subtask-input");
      // Fokus direkt in das neue Feld (Caret ans Ende)
      if (nextInput) {
        nextInput.focus();
        const v = nextInput.value || "";
        try { nextInput.setSelectionRange(v.length, v.length); } catch (_) {}
      }
    }
  });
...
function renderProfileBadge(){
  ...
  // Nur Vorname Nachname – KEINE Organisation in Klammern
  profileText.textContent = name;

  // Avatar (optional), sonst leer lassen (Standard: keines)
...
// Konto: Profilbild-Datei → Data-URL lesen & Vorschau
if (accountAvatarInput && !accountAvatarInput._wired){
  accountAvatarInput._wired = true;
  accountAvatarInput.addEventListener("change", (e)=>{
    ...
    reader.onload = () => {
      _profileAvatarData = String(reader.result || "");
      if (accountAvatarPreview) accountAvatarPreview.src = _profileAvatarData;
    };
    reader.readAsDataURL(file);
  });
}

// Konto-Panel: Speichern / Abbrechen
if (accountForm && !accountForm._wired){
  accountForm._wired = true;

  accountForm.addEventListener("submit", (e)=>{
    e.preventDefault();
    const first = (accountFirst?.value || "").trim();
    const last  = (accountLast?.value  || "").trim();
    if (!first || !last){
      (first ? accountLast : accountFirst)?.focus();
      return;
    }
    // org bewusst leer – nur Vorname/Nachname/Avatar
    prefs.profile = {
      ...(prefs.profile || {}),
      first, last,
      org: "",
    ...
    // zusätzlich: auch im users-Table speichern,
    // damit Chat & andere Apps darauf zugreifen können
    sbSaveUserProfile({
      first,
      last,
      avatar: _profileAvatarData || null
    }).catch(err => console.warn('sbSaveUserProfile failed', err));
  ...
  if (accountReset && !accountReset._wired){
    accountReset._wired = true;
    accountReset.addEventListener("click", (e)=>{
      e.preventDefault();
      populateAccountPane(); // Eingaben zurücksetzen
    });
  }
}
...
if (openProfileBtn && !openProfileBtn._wired){
  ...
  openProfileBtn.addEventListener("click", ()=>{
    closeDrawer();
    setTimeout(() => {
      switchSettingsView("account"); // öffnet direkt den Konto-Tab
...
// Drawer-Button: Zur Übersicht zurückkehren
const toOverviewBtn = document.getElementById("toOverviewBtn");
if (toOverviewBtn && !toOverviewBtn._wired) {
  toOverviewBtn._wired = true;
  toOverviewBtn.addEventListener("click", () => {
    closeDrawer();                     // Drawer animiert schließen
    setTimeout(() => {
...
[navDesign, navMisc, navAccount, navVersion].forEach(btn=>{
  ...
  btn.addEventListener("click", ()=>{
    switchSettingsView(btn.dataset.view);
  });
});

// Close-X
if (settingsCloseX && !settingsCloseX._wired){
  settingsCloseX._wired = true;
  settingsCloseX.addEventListener("click", ()=> closeModal(settingsModal));
}

// Konto → Profil öffnen
const openProfileFromSettings = q("#openProfileFromSettings");
if (openProfileFromSettings && !openProfileFromSettings._wired){
  openProfileFromSettings._wired = true;
  openProfileFromSettings.addEventListener("click", ()=>{
    // dein bestehendes Profil-Modal verwenden
    openProfileModal(true);
  });
}
...
// Konto-ID aus public.users einmalig holen und cachen
_accountIdCache = await sbGetAccountId();
