...
// ⬇️ Deine Werte (aus deinem bestehenden File beibehalten)
...
// Ermitteln, wo die Login-Seite liegt (Root vs. /app/)
const computeLoginPath = () => {
  const path = location.pathname;
  // Wenn wir schon im /app/-Ordner sind, ist login.html relativ dort
  if (path.includes("/app/")) return "login.html";
  // Sonst liegt sie unter /app/login.html
  return "./app/login.html";
};

const LOGIN_PATH = computeLoginPath();

// Session initial holen und global setzen
...
sb.auth.onAuthStateChange((event, session) => {
  ...
  if (event === "SIGNED_OUT" || !session?.user) {
    // Bei Logout immer zur Login-Seite, außer wir sind schon dort
    const now = location.pathname.toLowerCase();
    if (!now.endsWith("/login.html")) {
